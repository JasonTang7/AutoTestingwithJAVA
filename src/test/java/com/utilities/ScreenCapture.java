...
public class ScreenCapture {
	...
	private static Logger log =LogManager.getLogger("ScreenCapture");
	...
	public static void startVideoCapture()
	{
		try
		{
			if(new ConfigManager().getProperty("VideoCapture").equalsIgnoreCase("true") && new ConfigManager().getProperty("ModeOfExecution").equalsIgnoreCase("linear"))
			{
				...
				//log.info("VideosPath-----" + ReportSetup.getVideosPath());
	...
	public static void renameCapturedVideoFile(String sVideoFileName)
	{
		try
		{
		    ...
		    public int compare(Object o1, Object o2) 
		    {
		    	...
		    	return -1*(Long.valueOf(((File)o1).lastModified()).compareTo(Long.valueOf(((File)o2).lastModified()))); //for descending order 
		    }
		    });
		    String fileName = files[0].getName();
		    File oldfile = new File(ReportSetup.getVideosPath()+fileSeperator+fileName);
		    File newfile = null;
		    String newFileName = sVideoFileName + fileName.replace("ScreenRecording", "");
		    if(!new File(ReportSetup.getVideosPath()+fileSeperator+newFileName).exists())
		    {
		    	newfile = new File(ReportSetup.getVideosPath()+fileSeperator+newFileName);
		    }
		    else
		    {
		    	newfile = new File(ReportSetup.getVideosPath()+fileSeperator+newFileName.replace(".avi", "")+"(1).avi");
		    }
		    
		    //log.info("sVideoFileName----" + sVideoFileName + "---oldfile---" + oldfile + "---newfile---" + newfile + "---mediaFolder---" + mediaFolder); 
